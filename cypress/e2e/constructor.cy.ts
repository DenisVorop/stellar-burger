...
describe('Тестирование работы ингредиентов', () => {
    it('Добавление ингредиента из списка в конструктор', () => {
        cy.get(`[data-cy=${ITEM_MAIN_ID}]`).children('button').contains('Добавить').click();
        cy.get(`[data-cy=${ITEM_MAIN_ID}]`).find('.counter__num').should('contain', '1');
    });
...
describe('Тестирование оформления заказа', () => {
    it('Проверка оформления заказа', () => {
        cy.get(`[data-cy=${ITEM_MAIN_ID}]`).children('button').contains('Добавить').click();
        cy.get(`[data-cy=${ITEM_BUN_ID}]`).children('button').contains('Добавить').click();
