...
import { addIngredient, moveIngredient, removeIngredient, type ConstructorState } from '../reducers/constructor-slice';
...
describe('Проверка редьюсеров слайса constructor', () => {
    ...
    test('Удаление ингредиента', () => {
        ...
        const newState = constructorSlice(initialState, removeIngredient('643d69a5c3f7b9001cfa0941'));
    ...
    test('Изменение порядка ингредиентов в начинке', () => {
        ...
        const newState = constructorSlice(initialState, moveIngredient({ index: 0, newIndex: 1 }));
        const { ingredients } = newState;
        expect(ingredients).toEqual([
            {
                id: 'ziskw99vXUrTRTBKuqkjP',
